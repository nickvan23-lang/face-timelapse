...
class JobManager:
    def __init__(self):
        self.progress_q: queue.Queue = queue.Queue(maxsize=512)
        self.cancel_event = threading.Event()
        self.job_lock = threading.Lock()
        self.job_active: bool = False
        self.output_file: str = ""

    def reset_for_new_job(self, output_file: str) -> None:
        self.cancel_event = threading.Event()
...
def run() -> Response:
    ...
    with job_manager.job_lock:
        if job_manager.job_active:
            return jsonify({"error": "A job is already running."}), 409
        job_manager.job_active = True
    ...
    if not str(resolved_output).startswith(str(allowed_root)):
        with job_manager.job_lock:
            job_manager.job_active = False
        return jsonify({"error": "output_file must be within the current working directory."}), 400

    job_manager.reset_for_new_job(str(resolved_output))
    data["output_file"] = job_manager.output_file
...
def progress() -> Response:
    ...
    def generate():
        while True:
            try:
                event = job_manager.progress_q.get(timeout=30)
                try:
...
def video() -> Response:
    ...
    if not job_manager.output_file or not Path(job_manager.output_file).exists():
        return jsonify({"error": "No video available yet."}), 404
    return send_file(job_manager.output_file, mimetype="video/mp4")
...
def _run_pipeline(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        detections = detect_all_faces(
            ...
            cancel_event=job_manager.cancel_event,
        )

        if job_manager.cancel_event.is_set():
        ...
        job_manager.progress_q.put({"phase": "error", "pct": 0, "msg": str(exc), "eta": ""})
...
def _emit_cancelled(output_file: str) -> None:
    if output_file and Path(output_file).exists():
        try:
            ...
            pass
    job_manager.progress_q.put({"phase": "error", "pct": 0, "msg": "Cancelled by user.", "eta": ""})
